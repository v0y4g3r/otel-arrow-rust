...
pub fn from_record_messages(rbs: &[RecordMessage]) -> error::Result<(RelatedData, Option<usize>)> {
    ...
    for (idx, rm) in rbs.iter().enumerate() {
        ...
        match payload_type {
            ...
            ArrowPayloadType::ExpHistogramDpAttrs => {
                related_data.exp_histogram_attrs_store = Attribute32Store::try_from(&rm.record)?;
            }
    ...
    if let Some(number_dp_ex_rec_idx) = number_dp_ex_idx {
        ...
        related_data.number_data_point_exemplars_store =
            ExemplarsStore::try_from(record, &related_data.number_d_p_exemplar_attrs_store)?;
    }
    ...
    if let Some(histogram_dp_ex_rec_idx) = histogram_dp_ex_idx {
        ...
        related_data.histogram_data_point_exemplars_store =
            ExemplarsStore::try_from(record, &related_data.histogram_exemplar_attrs_store)?
    }
    ...
    if let Some(exp_histogram_dp_ex_rec_idx) = exp_histogram_dp_ex_idx {
        ...
        related_data.e_histogram_data_point_exemplars_store =
            ExemplarsStore::try_from(record, &related_data.exp_histogram_exemplar_attrs_store)?;
    }
    ...
    if let Some(summary_data_point_rec_idx) = summary_dp_idx {
        ...
        related_data.summary_data_points_store =
            SummaryDataPointsStore::from_record_batch(record, &related_data.summary_attrs_store)?;
    }
    ...
    if let Some(exp_histogram_data_point_idx) = exp_histogram_dp_idx {
        ...
        related_data.e_histogram_data_points_store = EHistogramDataPointsStore::from_record_batch(
            record,
